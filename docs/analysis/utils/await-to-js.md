...
function someAsyncFunction() {
  return new Promise(() => {
    throw new Error('wangkuo - try-catch')
  })
}
...
try {
  ...
  console.log(err.message) // 'wangkuo - try-catch'
}
```
...
function someAsyncFunction() {
  return new Promise(() => {
    throw new Error('wangkuo')
  })
}

const [err, data] = await to(someAsyncFunction())
err && console.log(err.message) // 'wangkuo'

/* 传递 errorExt 参数 */
const [err, data] = await to(someAsyncFunction(), { title: 'await-to-js' })
err && console.log(err.message, err.title) // 'wangkuo' 'await-to-js'
```
